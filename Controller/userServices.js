...
// @access Private
exports.sharpImageUser=asyncHandler(async(req,res,next)=>{
...
// @desc Create image user
// @route Post /api/v1/users
// @access Private
exports.postImageUser = createImgMiddleware('profileImg')
...
exports.updateUser = asyncHandler (async(req,res,next)=>{
  const docOfModel = await userModel.findByIdAndUpdate(req.params.id,
    {
      name:req.body.name,
      slug:req.body.slug,
    ...
    ,{new:true});
  if (!docOfModel) {
      return next(new ApiError(404,`${userModel} Cann't update for this: ${req.params.id}`))
  }
...
exports.updatePasswordAdmin = asyncHandler (async(req,res,next)=>{
  const docOfModel = await userModel.findByIdAndUpdate(req.params.id,
    {password:await bcrypt.hash(req.body.password, 10),passwordChangeAt:Date.now()}
    ,{new:true});
  if (!docOfModel) {
      return next(new ApiError(404,`${userModel} Cann't update Password for this: ${req.params.id}`))
  }
  console.log('done');
  res.status(200).json({data:docOfModel})
  })
